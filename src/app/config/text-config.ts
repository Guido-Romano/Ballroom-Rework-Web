...
interface Detalle extends ElementoComun {
  readonly info2: string;
  readonly imagen?: string;
}

interface Clase extends Detalle {
  readonly dia: string;
  readonly precio: string;
  readonly horario: string;
  readonly nivel: string;
  readonly dire: string;
}
...
interface Promocion extends Clase {}

interface Producto extends Detalle {
  readonly link: string;
}

interface Texto extends Detalle {}
...
const pregunta: Pregunta[] = [
  ...
  },
  {
    ...
    info: clase.map(c => c.dia).join(', ')
  },
  {
    id: 4,
    titulo: '¿Cuáles son los horarios de las clases?',
    info: clase.map(c => `${c.dia}, ${c.horario}`).join(' / ')
  },
  {
    id: 5,
    titulo: '¿Dónde se realizan las clases?',
    info: clase.map(c => c.dire).join(' / ')
  },
  {
    id: 6,
    titulo: '¿Cuál es el precio de las clases?',
    info: clase.map(c => c.precio).join(' / ') + ' al Mes.'
  },
  {
    id: 7,
